    ...
    public function index(Request $request)
    {
        ...
        if ($societeId) {
            ...
            foreach ($achatFiles as $file) {
                ...
                if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif'])) {
                    $file->preview = asset('storage/app/public/' . $file->path); // Image
                } elseif (in_array($extension, ['pdf'])) {
                    $file->preview = asset('storage/app/public/' . $file->path); // PDF
                } elseif (in_array($extension, ['doc', 'docx'])) {
                    $file->preview = asset('storage/app/public/' . $file->path); // Word
                } elseif (in_array($extension, ['xls', 'xlsx'])) {
                    $file->preview = asset('storage/app/public/' . $file->path); // Excel
                } else {
                    $file->preview = asset('storage/app/public/' . $file->path); // Fichier générique
                }
