...
<span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢{{ $dossier->name }}</span>
</div>
...
<div class="row" style="position: absolute; right: 0;">
        <div class="d-flex align-items-center mb-3">
            ...
            <h5 style="color: white; font-size: 12px;text-transform: capitalize;">{{ $dossier->name }}</h5>
         
         

            <form id="form-{{ $dossier->id }}" action="{{ route('Douvrir.upload') }}" method="POST" enctype="multipart/form-data">
                @csrf
<input type="file" name="file[]" id="file-{{ $dossier->name }}" multiple style="display: none;" onchange="handleFileSelect(event, '{{ $dossier->name }}')">                <input type="hidden" name="societe_id" value="{{ session()->get('societeId') }}">
                     <input type="hidden" name="type" value="{{ $dossier->name }}">
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
                ...
                <div class="">
                    <button 
                        ...
                        aria-expanded="false"
                        style="border: 1px solid white; border-radius: 10px; color: white; width: 100%;background-color:#4D55CC;margin-top:10px;">
                        Charger
                    </button>
...
function sortFiles() {
    ...
    function handleUpload(option) {
        if (option === 'importer') {
            document.getElementById('file-{{ $dossier->name }}').click();
...
formData.append('type', '{{ $dossier->name }}');
...
<!-- Modal Fusionner strictement aligné sur achat.blade.php -->
