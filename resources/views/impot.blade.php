...
<span  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢Impot</span>
</div>
...
<input type="file" name="file" id="file-impot" style="display: none;" onchange="handleFileSelect(event, 'impot')" multiple>
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
                ...
                <div class="">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButtonImpot">
                        <li><a class="dropdown-item" href="#" onclick="handleUploadImpot('importer')">Importer</a></li>
                        <!-- <li><a class="dropdown-item" href="#" onclick="handleUploadImpot('scanner')">Scanner</a></li> -->
                        <li><a class="dropdown-item" href="#" onclick="handleUploadImpot('fusionner')">Fusionner</a></li>
                    </ul>
                </div>
...
<input type="file" id="filesToMergeImpotHidden" multiple style="display: none;" onchange="mergeFilesImpotDirect(event)">
...
function sortFiles() {
 ...
 function handleUploadImpot(option) {
     if (option === 'importer') {
         document.getElementById('file-impot').click();
     } else if (option === 'scanner') {
         alert("Fonction de scan non implémentée.");
     } else if (option === 'fusionner') {
         document.getElementById('filesToMergeImpotHidden').click();
     }
 }

   async function mergeFilesImpotDirect(event) {
 ...
 async function confirmOrder() {
     closeModal();
     await mergeFilesImpot(confirmOrder);
 }

 async function mergeFilesImpot(files) {
...
<div id="fileOrderModal" style="display:none; position:fixed; top:30%; left:32%; transform:translate(-30%, -30%); background-color:white; border:1px solid #ccc; padding:20px; z-index:10000;width:90%;height:90%;">
    ...
    <div style="display: flex; align-items: flex-start; gap: 20px;height:120%;">
        ...
        <div style="display: flex; flex-direction: column; align-items: center; gap: 15px;">
            ...
            <span id="sortButton" onclick="sortFiles()" style="display: flex; align-items: center; justify-content: center; cursor: pointer; width: 60px; height: 60px; background-color: #cb0c9f; color: white; border-radius: 50%; font-size: 16px; user-select: none; padding: 5px;">
...
@else
    @php
        // Liste des dossiers automatiques pour lesquels on cache les options
        $autoFolders = [
            ...
            <div class="card shadow-sm" style="width: 10rem; height: 50px; cursor: pointer;">
                <div class="card-body text-center d-flex flex-column justify-content-between" style="padding: 0.5rem;background-color:#6f42c1;border-radius:17px;">
                    ...
                    <div class="d-flex justify-content-between" style="font-size: 0.8rem;">
                        @if (!in_array($folder->name, $autoFolders))
                            <!-- Menu contextuel uniquement si ce n'est pas un dossier automatique -->
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <li>
                                        <span class="dropdown-item text-muted" style="font-size: 0.85em; cursor: default;">
                                            Dernière modification : {{ \Carbon\Carbon::parse($folder->updated_at)->format('d/m/Y H:i') }}<br>
                                            par : {{ $folder->updatedBy ? $folder->updatedBy->name : ($folder->user ? $folder->user->name : 'Inconnu') }}
                                        </span>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="#" onclick="openRenameModal({{ $folder->id }}, '{{ $folder->name }}')">Renommer</a>
                                    </li>
                                    <li>
...
function openFile(folderId) {
    ...
    window.location.href = '/foldersImpot1/' + folderId;
}
