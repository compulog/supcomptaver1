...
<!-- <div>
    @if(session('dossierName'))
        <h4 style="color:#007bff;">Dossier ouvert : {{ session('dossierName') }}</h4>
    @endif
</div> -->
<br>
...
<a href="{{ route('exercices.show', ['societe_id' => session()->get('societeId')]) }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">Tableau De Board</a>
   <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;"> ➢{{$folder->type_folder}}</span>
    ...
    @foreach ($breadcrumbs as $index => $breadcrumb)
        @if ($index >= count($breadcrumbs) - 3)
            <a href="{{ route('folder.show', ['id' => $breadcrumb->id]) }}?breadcrumb={{ $breadcrumbTrailEncoded }}&clicked={{ urlencode($breadcrumb->name) }}&type={{$folder->type_folder}}"
...
<div class="row" style="position: absolute; right: 0;">
        <div class="d-flex align-items-center mb-3">
                ...
                <form id="form-{{ session('dossierName') }}" action="{{ route('uploadFileda') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="type" value="{{ session('dossierName') }}">
                <input type="file" name="file" id="file-{{ session('dossierName') }}" style="display: none;" onchange="handleFileSelect(event, '{{ session('dossierName') }}')">
                <input type="hidden" name="societe_id" value="{{ session()->get('societeId') }}">
<input type="hidden" name="folders" value="{{$foldersId}}">
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
                ...
                <div class="">
                    <button 
                        ...
                        aria-expanded="false"
                        style="background-color:#4D55CC;border: 1px solid white; border-radius: 10px; color: white; width: 100%;margin-top:10px;">
                        Charger
                    </button>
                ...
                <button type="submit" style="display: none;" id="submit-{{ session('dossierName') }}">Envoyer</button>
            </form>
            </div>
...
function sortFiles() {
   ...
   function handleUpload(option) {
    // Récupère le nom du dossier depuis la session côté Blade
    var dossierName = "{{ session('dossierName') }}";
    var fileInputId = 'file-' + dossierName;
    ...
    if (option === 'importer') {
        ...
        if (fileInput) {
            ...
            alert("Champ de fichier introuvable pour ce dossier.");
        }
...
formData.append('type', '{{ $folder->type_folder }}');
formData.append('folders', '{{ $foldersId }}'); // Ajout de l'ID du dossier
