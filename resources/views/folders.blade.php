...
<div style="position: fixed; z-index: 1000;margin-left:1%;margin-top:3%;">
 ...
 <span  style="color:rgb(34, 146, 245); text-decoration: underline;">➢</span>   
    <a href="{{ route('achat.view') }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">Achat</a>
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
                ...
                <button type="submit" style="display: none;" id="submit-achat">Envoyer</button>
                    ...
                    <div class="p-0" style="background-color: transparent; border-radius: 15px; font-size: 0.75rem; display: inline-flex; justify-content: center; align-items: center; height: auto; width: auto;">
                    @php
                        $currentFilter = request()->get('filter_by', 'name'); // name | date
                        $currentOrder  = request()->get('order_by', 'asc');  // asc | desc
                    @endphp

                    <form method="GET" action="{{ route('folder.show', $folder->id) }}" class="d-flex me-3">
                        <div class="input-group" style="align-items: center; gap: 6px;">
...
function sortFiles() {
  ...
  function moveFile(fromIndex, toIndex) {
    ...
    const fileToMove = selectedFiles.splice(fromIndex, 1)[0];
    const rotationToMove = rotations.splice(fromIndex, 1)[0];

    selectedFiles.splice(toIndex, 0, fileToMove);
    rotations.splice(toIndex, 0, rotationToMove);
...
//   function sortFiles() {
//     const combined = selectedFiles.map((file, i) => ({ file, rotation: rotations[i] }));
//     combined.sort((a, b) => a.file.name.localeCompare(b.file.name));
//     selectedFiles = combined.map(c => c.file);
//     rotations = combined.map(c => c.rotation);
//     populateFileList();
//   }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (fileType === 'application/pdf') {
        ...
        copiedPages.forEach((page) => {
          if (rotation !== 0) {
            const angle = rotation === 90 ? PDFLib.degrees(90) :
                          rotation === 180 ? PDFLib.degrees(180) :
                          rotation === 270 ? PDFLib.degrees(270) : PDFLib.degrees(0);
...
formData.append('file', blob, `${safeFileName}_${Date.now()}.pdf`);    formData.append('societe_id', '{{ session()->get("societeId") }}');
...
<style>
  ...
  #fileOrderModal ul li span[onclick] {
    cursor: pointer;
    padding: 0 5px;
    color: #007BFF;
    font-size: 14px;
  ...
  #fileOrderModal ul li span[onclick]:hover {
    color: #0056b3;
  }
                    ...
                    <div class="card shadow-sm" style="width: 13rem; height: 250px;">
                        <div class="card-body text-center d-flex flex-column justify-content-between" style="padding: 0.5rem;">
                            ...
                            @if(in_array(strtolower(pathinfo($file->name, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif']))
                                <img src="{{ asset($file->path) }}" alt="{{ $file->path }}" class="img-fluid mb-2" style="overflow-clip-margin: content-box;overflow: clip; height: 200px">
...
function searchAndHighlightText(text) {
    ...
    for (let i = 0; i < textElements.length; i++) {
        ...
        if (textElements[i].textContent.trim().includes(text.trim())) {
            console.log("Texte trouvé à l'index:", i);

            // Appliquer le surlignage avec un pseudo-élément ::after
            textElements[i].classList.add('highlight');
            found = true;
            break;  // Si vous voulez juste surligner la première occurrence
        }
    }
 ...
 <style>
/* Surlignage avec fond jaune et texte en noir */
.highlight {
    position: relative;
}
