...
.form-select {
    ...
    <a href="{{ route('impot.view') }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">Impot</a>
    <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢</span> 
    ...
    @foreach ($breadcrumbs as $index => $breadcrumb)
        @if ($index >= count($breadcrumbs) - 3)
            <a href="{{ route('folder.show', ['id' => $breadcrumb->id]) }}?breadcrumb={{ $breadcrumbTrailEncoded }}&clicked={{ urlencode($breadcrumb->name) }}&type=impot"
    ...
    <div style="display: flex; align-items: flex-start; gap: 20px;height:120%;">
        ...
        <div style="display: flex; flex-direction: column; gap: 10px; align-items: center;">
            ...
            <button onclick="confirmOrder()" 
                style="background:#4D55CC; color:#fff; border:none; border-radius:10px; font-size:15px; font-weight:600; padding:10px 0; width:100px; margin-top:10px; box-shadow:0 2px 8px rgba(77,85,204,0.08); transition:background 0.2s;"
                onmouseover="this.style.background='#2e3a8c'" 
                onmouseout="this.style.background='#4D55CC'">
                Valider
            </button>
        </div>
    </div>
...
<div class="modal fade" id="createFolderModal" tabindex="-1" aria-labelledby="createFolderModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('foldersImpot1.create') }}" method="POST">
                    @csrf
