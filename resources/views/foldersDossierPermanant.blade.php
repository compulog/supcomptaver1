...
<!-- Ajoute ce script dans le <head> ou avant tout <script> qui utilise Swal -->
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
   ...
   <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢</span> 
    ...
    <a href="{{ route('Dossier_permanant.view') }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">Dossier-Permanant</a>
   <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢</span> 
    ...
    @foreach ($breadcrumbs as $index => $breadcrumb)
        @if ($index >= count($breadcrumbs) - 3)
            <a href="{{ route('folder.show', ['id' => $breadcrumb->id]) }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">{{ $breadcrumb->name }}</a>
            @if (!$loop->last)    <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢</span> 
 @endif
        @endif
    @endforeach
...
// Place ce bloc AVANT le HTML qui utilise handleUploadDossier_permanant, ou dans un <script> dans le <head>
window.handleUploadDossier_permanant = function(option) {
    if (option === 'importer') {
        document.getElementById('file-dossier_permanant').click();
    } else if (option === 'scanner') {
        alert("Fonction de scan non implémentée.");
    } else if (option === 'fusionner') {
        document.getElementById('filesToMergeDossier_permanantHidden').click();
    }
};
...
<input type="file" name="file" id="file-dossier_permanant" style="display: none;" onchange="handleFileSelect(event, 'dossier_permanant')" multiple>
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
                ...
                <div class="">
                    <button 
                    class="btn btn-primary btn-sm" style="background-color:#4D55CC;height: 38px; margin-right: 10px;" 
                                            type="button" 
                        id="dropdownMenuButtonDossier_permanant" 
                        data-bs-toggle="dropdown" 
                        aria-expanded="false"
                       >
                        Charger
                    </button>
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButtonDossier_permanant">
                        <li><a class="dropdown-item" href="#" onclick="handleUploadDossier_permanant('importer')">Importer</a></li>
                        <!-- <li><a class="dropdown-item" href="#" onclick="handleUploadDossier_permanant('scanner')">Scanner</a></li> -->
                        <li><a class="dropdown-item" href="#" onclick="handleUploadDossier_permanant('fusionner')">Fusionner</a></li>
                    </ul>
                </div>
...
<input type="file" id="filesToMergeDossier_permanantHidden" multiple style="display: none;" onchange="mergeFilesDossier_permanant()">
...
function handleUploadDossier_permanant(option) {
    if (option === 'importer') {
        document.getElementById('file-dossier_permanant').click(); // <-- corrige ici
    } else if (option === 'scanner') {
        alert("Fonction de scan non implémentée.");
    } else if (option === 'fusionner') {
        document.getElementById('filesToMergeDossier_permanantHidden').click();
   ...
   async function mergeFilesDossier_permanantDirect(event) {
 ...
 function rotateImage(preview, index) {
     ...
     img.onload = function () {
         // Pour redessiner avec rotation correcte, on préfère relire l'image originale :
         // Mais ici on a juste le canvas (preview) en base64, donc on redessine à partir du preview:
         // Mieux vaut refaire la lecture du fichier, on va forcer rechargement complet en appelant populateFileList()
         // pour simplifier on peut appeler populateFileList() pour recharger toutes les images avec rotation correcte.
         populateFileList();
     };
 }
 ...
 async function confirmOrder() {
     closeModal();
     await mergeFilesDossier_permanant();
 }

 async function mergeFilesDossier_permanant() {
     try {
       ...
       const fileNameBase = selectedF iles.map(f => f.name.replace(/\.[^/.]+$/, '')).join('_');
   ...
   const foldersInput = document.querySelector('input[name="folders"]');
    if (foldersInput) {
      formData.append('folders', foldersInput.value);
         ...
         if (data.success) {
             // ✅ Affichage message temporaire de succès
             ...
             // ⏱️ Attente de 2 secondes avant rechargement
             setTimeout(() => {
                 location.reload();
             }, 2000);
         } else {
            ...
            @foreach ($folders as $folder)
                <div class="col" ondblclick="openFile({{ $folder->id }})">
                    <div class="card shadow-sm" style="width: 10rem; height: 50px; cursor: pointer;">
                        <div class="card-body text-center d-flex flex-column justify-content-between" style="padding: 0.5rem;background-color:rgb(221, 232, 17);border-radius:17px;">
                            <!-- Icône du Dossier -->
...
showSuccessNotification("Importation en cours...", true);
    ...
    .then(response => response.json())
     .then(data => {
        removeSuccessNotification(); // Retire la notification "en cours"
