...
<input type="file" name="file" id="file-achat" style="display: none;" onchange="handleFileSelect(event, 'achat')" multiple>
                ...
                <div class="dropdown">
                    <button 
                        ...
                        aria-expanded="false"
                        style="border: 1px solid white; border-radius: 10px; color: white; width: 100%;">
                        Charger
                    </button>
...
<!-- <input type="file" id="filesToMergeHidden" multiple style="display: none;" onchange="mergeFilesDirect(event)"> -->
...
<!-- <script>
    ...
    async function mergeFilesDirect(event) {
        try {
            ...
            if (data.success) {
                // ✅ Message temporaire au lieu d'alerte
                ...
                // ⏱️ Attente 2 secondes avant rechargement
                setTimeout(() => {
                    location.reload();
                }, 2000);
            } else {
...
async function mergeFiles(files) {
    ...
    const formData = new FormData();
    formData.append('file', blob, `merged_${Date.now()}.pdf`);
...
function handleFileSelect(event, type) {
    ...
    for (let i = 0; i < fileInput.files.length; i++) {
        ...
        formData.append('files[]', file); // Utilisez 'files[]' pour un tableau de fichiers
    }
    ...
    .then(data => {
        if (data.exists) {
            if (confirm(`Le fichier avec le nom "${data.name}" existe déjà. Voulez-vous continuer ?`)) {
                // Réimporter avec force=1
