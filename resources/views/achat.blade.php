...
<div id="loader" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; 
    background: rgba(255,255,255,0.8); z-index:9999; text-align:center; padding-top:200px; font-size:20px;">
...
<div class="container mt-4" style="position: fixed; z-index: 1000;">
...
<div class="row" style="position: absolute; right: 0;">
        ...
        <div class="d-flex align-items-center mb-3">
            ...
            <form id="form-achat" action="{{ route('uploadFile') }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="file" name="file" id="file-achat" style="display: none;" onchange="handleFileSelect(event, 'achat')" multiple>
                ...
                <div class="">
                    <button 
                        ...
                        aria-expanded="false"
                        style="border: 1px solid white; border-radius: 10px; color: white; width: 100%;background-color:#4D55CC;">
                        Charger
                    </button>
  ...
  <script>
    ...
    // Extraire et concaténer les noms de fichiers sans extension
    ...
    formData.append('file', blob, `${safeFileName}_${Date.now()}.pdf`);
        formData.append('societe_id', '{{ session()->get('societeId') }}'); // Laravel dynamique
...
@php
    $currentFilter = request()->get('filter_by', 'name'); // Nom ou Date
    $currentOrder = request()->get('order_by', 'asc');    // asc ou desc
@endphp

<form method="GET" action="" class="d-flex me-3">
    <div class="input-group" style="align-items: center; gap: 5px;">

        <!-- Label Triée par -->
        <span style="height: 38px; min-width: 100px; cursor: default; color:#4D55CC; 
        ...
        <div style="display: inline-block; position: relative; margin-top: -20px; margin-left: -15px;">

            <!-- Select Nom/Date -->
            <select name="filter_by" class="form-select"
                    style="height: 38px; width: 70px; font-size: 0.875rem; margin-left:5px;
                           color:#4D55CC; border-color:#4D55CC;"
            ...
            <i class="fas fa-chevron-down"
               style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%);
                      pointer-events: none; color: #4D55CC; font-size:10px;"></i>
        </div>
        ...
        <button type="submit"
            ...
            <!-- Flèche -->
            <span style="font-size: 16px;">{{ $currentOrder == 'asc' ? '↑' : '↓' }}</span>
        ...
        <!-- ❌ Hidden filter removed (cause of bug) -->
...
<div class="search-icon" id="searchIcon" 
     style="margin-top:-20px;width:50px; cursor:pointer; padding:10px; 
                ...
                <div class="col" ondblclick="viewFile({{ $file->id }})">
                    <div class="card shadow-sm" style="width: 13rem; height: 250px;">
                        <div class="card-body text-center d-flex flex-column justify-content-between" style="padding: 0.5rem;">
                            ...
                            <div class="d-flex justify-content-between" style="font-size: 0.8rem;">
                                <!-- Menu contextuel -->
                                <div style="margin-top:-220px;margin-left:190px;">
                                    <button class="btn btn-link p-0" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" style="margin-top:-20px;">
                                        <i class="fas fa-ellipsis-v" style="color:#4D55CC;"></i>
                                    </button>
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                ...
                <script>
                    ...
                    document.addEventListener("DOMContentLoaded", function() {
                        @if(strtolower(pathinfo($file->name, PATHINFO_EXTENSION)) == 'pdf')
                            ...
                            // Utilisation de PDF.js pour afficher toutes les pages du PDF
                            pdfjsLib.getDocument(url).promise.then(function (pdf) {
                                for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
                                    pdf.getPage(pageNum).then(function (page) {
                                        ...
                                        document.getElementById('filePreviewContent').appendChild(canvas); // Ajoutez le canvas au contenu du fichier
...
<div class="action-buttons" style="display: flex; align-items: center; gap: 10px; position: absolute; top: 15px; right: 11%; z-index: 1000;">
...
// Remplissez le tableau 'files' avec les fichiers à afficher
// Exemple : files = [{id: 1, name: 'file1.pdf'}, {id: 2, name: 'file2.pdf'}, ...];
...
function handleFileSelect(event, type) {
    ...
    formData.append('folders_id', 0);
    formData.append('exercice_debut', '{{ $societe->exercice_social_debut }}');
formData.append('exercice_fin', '{{ $societe->exercice_social_fin }}');

    // 👉 Affiche la notification verte "Importation en cours..."
...
pdfjsLib.getDocument({ url: fileURL }).promise.then(async pdf => {
  ...
  function loadFilePreviewContent(fileId) {
    fetch(`/file/${fileId}/preview`)
        .then(response => response.text())
        .then(filePreviewContent => {
            document.getElementById("filePreviewContent").innerHTML = filePreviewContent;
            document.getElementById("filePreviewContent").addEventListener("mouseup", function () {
                const selection = window.getSelection();
                if (selection.toString().length > 0) {
                    selectedText = selection.toString();  // Stocker le texte sélectionné
                    ...
                    if (!commentIcon) {
                        ...
                        // Superposer les icônes
                        iconStack.appendChild(commentBase);
                        iconStack.appendChild(plusIcon);

                        // Ajouter l'icône au div principal
                        commentIcon.appendChild(iconStack);

                        // Ajouter un écouteur d'événement pour la clique sur l'icône
                        commentIcon.addEventListener("click", handleCommentClick);
                    }
                }
            });

            // Charger les commentaires
            fetch(`/file/${fileId}/comments`)
                .then(response => response.json())
                .then(comments => {
    ...
    comments.forEach(comment => {
        ...
        commentDiv.innerHTML = `
            ...
            <div class="comment-actions">
                <button class="edit-comment-btn" data-comment-id="${comment.id}">Modifier</button>
