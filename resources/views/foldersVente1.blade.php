...
<h6 style="margin-top: 1.5rem !important;">
    ...
    @foreach ($breadcrumbs as $index => $breadcrumb)
        @if ($index >= count($breadcrumbs) - 3)
            <a href="{{ route('folder.show', ['id' => $breadcrumb->id]) }}?breadcrumb={{ $breadcrumbTrailEncoded }}&clicked={{ urlencode($breadcrumb->name) }}&type=vente"
               style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">
               {{ $breadcrumb->name }}
            </a>
            @if (!$loop->last)
...
function sortFiles() {
 ...
 async function mergeFilesVente(files) {
...
<div id="fileOrderModal" style="display:none; position:fixed; top:30%; left:32%; transform:translate(-30%, -30%); background-color:white; border:1px solid #ccc; padding:20px; z-index:10000; width:90%; height:90%;">
    ...
    <div style="display: flex; align-items: flex-start; gap: 20px; height: calc(100% - 60px);">
        <ul id="fileList" style="list-style-type:none; padding:0; margin:0; max-height: 100%; overflow-y: auto; border: 1px solid #eee; border-radius: 4px; padding: 10px; background-color: #f9f9f9; flex: 1;"></ul>
        ...
        <div style="display: flex; flex-direction: column; justify-content: space-between; align-items: center;">
            ...
            <!-- Bouton Valider repositionné correctement -->
            <button onclick="confirmOrder()" 
                style="background-color: #007BFF; color: white; padding: 6px 12px; font-size: 14px; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.2s; align-self: flex-start;">
                Valider
            </button>
        </div>
    </div>
...
<input type="hidden" id="folders-vente-hidden" value="{{ session()->get('foldersId') }}">
...
<div class="modal fade" id="createFolderModal" tabindex="-1" aria-labelledby="createFolderModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('folderVente.create') }}" method="POST">
                    @csrf
