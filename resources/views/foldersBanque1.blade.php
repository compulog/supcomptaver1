...
.form-select {
    ...
    <a href="{{ route('banque.view') }}"  style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">Banque</a>
     <span style="color:rgb(34, 146, 245); text-decoration: underline; font-weight: bold;">➢</span> 
    ...
    @foreach ($breadcrumbs as $index => $breadcrumb)
        @if ($index >= count($breadcrumbs) - 3)
            <a href="{{ route('folder.show', ['id' => $breadcrumb->id]) }}?breadcrumb={{ $breadcrumbTrailEncoded }}&clicked={{ urlencode($breadcrumb->name) }}&type=banque"
...
<input type="file" name="file" id="file-banque" style="display: none;" onchange="handleFileSelect(event, 'banque')" multiple>
...
<input type="hidden" name="exercice_fin" value="{{ $societe->exercice_social_fin }}">
        ...
        <div class="">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButtonBanque">
                <li><a class="dropdown-item" href="#" onclick="handleUploadBanque('importer')">Importer</a></li>
                <!-- <li><a class="dropdown-item" href="#" onclick="handleUploadBanque('scanner')">Scanner</a></li> -->
                <li><a class="dropdown-item" href="#" onclick="handleUploadBanque('fusionner')">Fusionner</a></li>
            </ul>
        </div>
...
<select name="order_by" class="form-select" style="height: 38px; width: auto; max-width: 200px; font-size: 0.875rem;">
  ...
  <input type="file" id="filesToMergeBanqueHidden" multiple style="display: none;" onchange="mergeFilesBanqueDirect(event)">
...
function handleUploadBanque(option) {
    if (option === 'importer') {
        ...
        document.getElementById('filesToMergeBanqueHidden').click();
    }
}

async function mergeFilesBanqueDirect(event) {
...
<input type="hidden" id="folders-banque-hidden" value="{{ session()->get('foldersId') }}">
