...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['permissions'], function (Blueprint $table) {
            ...
            $table->string('name', 100); // Limit to 191 characters
            $table->string('guard_name', 100); // Limit to 191 characters
            $table->timestamps();

            $table->unique(['name', 'guard_name']); // Unique index
        });

        // Create roles table
        Schema::create($tableNames['roles'], function (Blueprint $table) use ($teams, $columnNames) {
            $table->bigIncrements('id'); // Role ID
            if ($teams || config('permission.testing')) { // Permission testing fix for SQLite
