...
var contrePartieColumn = {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeSection(section) {
        ...
        const 
        Container = document.getElementById(`periode-${section}`);
...
let toggle = false;
  ...
  ajaxResponse: function(url, params, response) {
   ...
   editor: function (cell, onRendered, success, cancel) {
    ...
    const exerciceRadio = document.getElementById("filter-exercice-achats");
    const moisRadio = document.getElementById("filter-mois-achats");

    // Définition placeholder selon radio
    if (exerciceRadio && exerciceRadio.checked) {
        input.placeholder = "jj/mm";

        // Fonction pour forcer la présence du slash au 3e caractère
        function fixSlash() {
            let val = input.value;
            // Supprimer tout slash
            val = val.replace(/\//g, "");
            // Limiter la longueur à 4 caractères (jjmm)
            if (val.length > 4) val = val.slice(0, 4);
            // Ajouter slash après les 2 premiers caractères si possible
            if (val.length > 2) {
        ...
        // Fixer slash au chargement de la valeur initiale
        ...
        if (currentValue) {
            ...
            if (dt.isValid) input.value = dt.toFormat("dd/MM");
        }

        input.addEventListener("input", fixSlash);

        // Positionner curseur après jj/
        onRendered(() => {
            input.focus();
    ...
    } else if (moisRadio && moisRadio.checked) {
        input.placeholder = "jj/";

        // valeur initiale au format dd/MM/yyyy
        const currentValue = cell.getValue();
        if (currentValue) {
            let dt = luxon.DateTime.fromFormat(currentValue, "yyyy-MM-dd HH:mm:ss");
            if (!dt.isValid) dt = luxon.DateTime.fromISO(currentValue);
    ...
    function validateAndCommit() {
        ...
        const isExerciceEntier = exerciceRadio && exerciceRadio.checked;
        const isMois = moisRadio && moisRadio.checked;

        const val = input.value.trim();
        const parts = val.split("/");

        if (isExerciceEntier && parts.length !== 2) {
            alert("Format invalide : le format attendu est jj/mm");
            cancel();
            return false;
        } else if (isMois && parts.length < 1) {
            // validation spécifique mois si besoin
        } else if (!isExerciceEntier && !isMois && parts.length !== 3) {
            alert("Format invalide : le format attendu est jj/mm/aaaa");
            cancel();
            return false;
        }

        const day = parseInt(parts[0], 10);
        const month = isExerciceEntier ? parseInt(parts[1], 10) : (parts[1] ? parseInt(parts[1], 10) : 1);
        const year = isExerciceEntier ? parseInt(anneeInput.value, 10) : (parts[2] ? parseInt(parts[2], 10) : parseInt(anneeInput.value, 10));

        if (isNaN(day) || isNaN(month) || isNaN(year)) {
            alert("Date invalide : jour, mois ou année incorrecte");
            cancel();
            return false;
        }
        ...
        const lignesFacture = rows.filter(r => r.getData().numero_facture === numeroFacture);
        ...
        if (totalDebit === totalCredit && totalDebit !== 0) {
            ...
            const prefix = `P${moisFormatted}${codeJournalInput.value || "CJ"}`;
            ...
            const pieceJustificative = `P${moisFormatted}${codeJournalInput.value || "CJ"}${numeroFormate}`;

            currentRow.update({ piece_justificative: pieceJustificative });
            console.log("Nouvelle pièce générée :", pieceJustificative);
        }

        currentRow.update({ date_livraison: dt.toFormat("yyyy-MM-dd HH:mm:ss") });
...
// ← NOUVELLE colonne Date livraison
{
    ...
    editor: function(cell, onRendered, success, cancel) {
        ...
        if (raw) {
            let dt = luxon.DateTime.fromFormat(raw, "dd/MM/yyyy");
            if (!dt.isValid) dt = luxon.DateTime.fromFormat(raw, "yyyy-MM-dd HH:mm:ss");
            if (!dt.isValid) dt = luxon.DateTime.fromISO(raw);
            if (dt.isValid) input.value = dt.toFormat("dd/MM/yyyy");
        }

        // ✅ Ajout du datepicker Flatpickr
        onRendered(() => {
            input.focus();
            flatpickr(input, {
                dateFormat: "d/m/Y",       // affichage dans le champ
                altInput: false,           // pas de champ alternatif
                allowInput: true,          // permet l'écriture manuelle
                defaultDate: input.value,  // valeur initiale
                locale: "fr"
            });
        });
        ...
        function validate() {
            ...
            if (parts.length !== 3) { cancel(); return; }
            const [j, m, a] = parts.map(n => parseInt(n, 10));
            const dt = luxon.DateTime.local(a, m, j);
            if (!dt.isValid) {
                alert("Date invalide (format requis : jj/MM/aaaa)");
                cancel();
                return;
            }
            // ✅ Stocker en format strict ISO
            success(dt.toFormat("yyyy-MM-dd HH:mm:ss"));
        }
    ...
    // ✅ Formatter affichant toujours en 08/01/2025
    formatter: function(cell) {
        const raw = cell.getValue();
        if (!raw) return "";
        let dt = luxon.DateTime.fromFormat(raw, "yyyy-MM-dd HH:mm:ss");
        if (!dt.isValid) dt = luxon.DateTime.fromFormat(raw, "dd/MM/yyyy");
        if (!dt.isValid) dt = luxon.DateTime.fromISO(raw);
...
// Configuration de la colonne "Compte" dans Tabulator
{
  ...
  formatter: function(cell) {
    ...
    if (inputEl) {
      // Synchroniser la saisie dans l'input avec la donnée Tabulator
      inputEl.addEventListener("input", function(e) {
        const newValue = e.target.value;
        // Met à jour la donnée dans Tabulator (important pour que ce soit pris en compte dans getData)
        cell.setValue(newValue);
      });

      // Gestion du focus/tab/enter pour ergonomie
  ...
  cellClick: function(e, cell) {
    if (!e.target.classList.contains('upload-icon')) return;
...

{
  ...
  titleFormatter: function () {
    // Crée manuellement l’élément input
  ...
  formatter: function (cell) {
    ...
    // met à jour master checkbox si nécessaire
    function updateMasterCheckbox() {
      const allRows = tableAch.getRows();
      const selectedRows = tableAch.getSelectedRows();
      const master = document.getElementById("master-select");
      if (!master) return;
      master.checked = allRows.length && selectedRows.length === allRows.length;
    }
    ...
    row.getTable().on("rowSelectionChanged", updateMasterCheckbox); // synchronise

    wrap.appendChild(cb);

    // b) bouton 🔄 (rafraîchir données)
    const btnR = document.createElement("button");
    btnR.type = "button";
    btnR.title = "Rafraîchir";
    btnR.textContent = "🔄";
    ...
    btnR.addEventListener("click", (e) => {
      e.stopPropagation();
      tableAch.replaceData(); // ou replaceData([...]) si besoin
    });
    wrap.appendChild(btnR);
        ...
        btnS.addEventListener("click", (e) => {
          e.stopPropagation();
          enregistrerLignesAch(); // Appel de la fonction d'enregistrement
      ...
      cellClick: function (e, cell) {
        if (e.target.tagName === "INPUT") e.stopImmediatePropagation();
      },
      cellEdited: function (cell) {
...
// ⏱️ Intervalle pour forcer le focus
...
// ✅ Attacher l’événement une seule fois sur la table (et non via getTable().once)
const table = cell.getRow().getTable();

// On vérifie que l'événement n’a pas déjà été attaché (pour éviter les doublons)
if (!table._editingListenerAttached) {
    table.on("cellEdited", function() {
        // Nettoyage après édition
        localStorage.removeItem("currentEditingCell");
        window.removeEventListener("visibilitychange", reFocusIfNeeded);
        clearInterval(focusInterval);
    });
    table._editingListenerAttached = true; // Marquer comme attaché
}
...
// Exemple dans ecouterEntrer(tableAch) :
tableAch.element.addEventListener("keydown", async function(event) {
  if (event.key !== "Enter") return;
  event.preventDefault();

  const activeRow = tableAch.getSelectedRows()[0]?.getData();
  if (activeRow && activeRow.date) {
    const annee = activeRow.date.slice(0, 4);
    if (await isExerciceCloture(annee)) {
      alert(`L'exercice ${annee} est déjà clôturé, impossibilité de saisir de nouvelles lignes.`);
      return;
    }
  }
...
// Écouteur pour l'input PDF Achats
// Upload et injection
...
//     try {
//       const res  = await fetch('/operation-courante/pdf-extract', {
//         method: 'POST',
...
//       await tableAch.setData(data);
//     } catch (err) {
//       alert('Erreur IA PDF : ' + err.message);
//     } finally {
...
async function ajouterLigne(table, preRemplir = false, ligneActive = null) {
  ...
  if (!filterAchats) {
    alert("Veuillez sélectionner un mode de saisie : Contre-partie Auto ou Libre.");
    return [];
  }
  ...
  if (preRemplir && ligneActive) {
    ...
    // Copie pour y stocker le résultat de calcul
    const temp = { ...ligneActive, credit: creditInit };

    // 2) Calcul TVA + chargeHT
    await calculerDebit(temp, filterAchats === "contre-partie");

    //  temp.lignesTVA          = [ { compte_tva, taux_tva, debit_tva }, … ]
    //  temp.debit_contrepartie  = montantChargeHT
    //  temp.compte_debit_charge = "61320000" (par exemple)

    // 3) Ligne fournisseur (crédit TTC)
    nouvellesLignes.push({
      ...ligneActive,
      id: idCounter++,
      compte: ligneActive.compte,               // ex: "4411000002"
    ...
    // 4) Ligne charge HT (débit)
    if (temp.compte_debit_charge && Number.isFinite(temp.debit_contrepartie)) {
      nouvellesLignes.push({
        ...ligneActive,
        id: idCounter++,
        compte: temp.compte_debit_charge,       // ex: "21110000"
        contre_partie: ligneActive.compte,      // ex: "4411000002"
        debit: temp.debit_contrepartie,         // ex: 5000
        credit: 0,
        type_journal: codeJournal,
      });
    } else {
      console.warn("⚠️ Aucun compte_debit_charge ou débit invalide → pas de ligne chargeHT");
    }

    // 5) Ligne(s) TVA (débit)
    if (Array.isArray(temp.lignesTVA) && temp.lignesTVA.length > 0) {
      temp.lignesTVA.forEach((tvaLine) => {
        nouvellesLignes.push({
          ...ligneActive,
          id: idCounter++,
          compte: tvaLine.compte_tva || "",      // ex: "3455000" ou vide
          contre_partie: ligneActive.compte,     // ex: "4411000002"
          debit: tvaLine.debit_tva,              // ex: 1000
          credit: 0,
          type_journal: codeJournal,
        });
      });
    } else {
      console.warn("⚠️ Aucune ligne TVA générée pour cette facture");
    }
  } else {
    // Mode “libre” ou pas de préremplissage → on ajoute une ligne vide
    nouvellesLignes.push(ajouterLigneVide(idCounter, null, codeJournal, null));
  }

  // 6) Injection dans Tabulator
  nouvellesLignes.forEach((l) => table.addRow(l, false));
...
function generatePiece(dateStr, codeJournal) {
  ...
  if (!dt.isValid) dt = luxon.DateTime.fromFormat(dateStr, "yyyy-MM-dd HH:mm:ss");
  if (!dt.isValid) dt = luxon.DateTime.fromFormat(dateStr, "yyyy-MM-dd");
  if (!dt.isValid) {
    console.warn(`❌ Date invalide pour génération de pièce : ${dateStr}`);
    return "";
  }
  const MM = dt.toFormat("MM");
  const YY = dt.toFormat("yy");
...
function updatePieceJustificative(lignes, codeJournal) {
  console.log("🔍 Données brutes :", JSON.parse(JSON.stringify(lignes)));

  // Initialiser le compteur sur base des pièces existantes
  ...
  console.log("🔍 Factures :", Object.keys(groupes));

  // Pour chaque facture, réutiliser ou générer la pièce
  Object.entries(groupes).forEach(([nf, group]) => {
    const exist = group.find(l => l.piece_justificative)?.piece_justificative;
    if (exist) {
      console.log(`🔁 Réutilise pièce pour ${nf}: ${exist}`);
      group.forEach(l => l.piece_justificative = exist);
    } else {
      const sample   = group[0];
      const dateOnly = sample.date.split(" ")[0];
      const piece    = generatePiece(dateOnly, codeJournal);
      console.log(`✅ Génère pièce pour ${nf}: ${piece}`);
      group.forEach(l => l.piece_justificative = piece);
    }
  });

  console.log("🔚 Données après update :", JSON.parse(JSON.stringify(lignes)));
  return lignes;
}

/**
 * Fonction complète d'enregistrement des lignes :
 * 1) Génération des pièces
 * 2) Rafraîchissement Tabulator
 * 3) Préparation & envoi AJAX
 * 4) Rechargement & ajout de ligne vide
 * 5) Recalcul du solde
 */
// Compteur global pour les pièces
const compteurMap = {};

/** (génèrePiece & updatePieceJustificative restent inchangées) */

/**
 * Enregistre les lignes avec génération de pièces
 */
async function enregistrerLignesAch() {
  try {
    // 1) Récupérer et normaliser le codeJournal
    ...
    // 2) Vider le compteur pour repartir de zéro
    for (const key in compteurMap) delete compteurMap[key];

    // 3) Récupérer les lignes saisies
    let lignes = tableAch.getData().filter(l => l && typeof l === "object");

    // 4) Mettre à jour les pièces en passant codeJournal
    lignes = updatePieceJustificative(lignes, codeJournal);

    // 5) Synchroniser Tabulator avant envoi
    ...
    // 8) Récupérer la réponse et recharger la table
    const result    = await response.json();
    const dataToSet = Array.isArray(result) ? result : (result.data || []);
    tableAch.setData(dataToSet);
    tableAch.redraw(true);

    // 9) **Reconstruire le compteur** à partir des données fraîchement chargées
    //    pour pouvoir incrémenter correctement au prochain enregistrement
    dataToSet.forEach(l => {
      if (l.piece_justificative) {
        const m = l.piece_justificative.match(/^P(\d{2})(\d{2})([A-Za-z0-9]+)(\d{4})$/);
        if (m) {
          const key = `P${m[1]}${m[2]}${m[3]}`;
          const num = parseInt(m[4], 10);
          compteurMap[key] = Math.max(compteurMap[key] || 0, num);
    ...
    // 10) Ajouter une ligne vide si besoin
    const current = tableAch.getData();
    if (current.length && current[current.length-1].compte) {
      tableAch.addRow(ajouterLigneVide(current.length+1, null, codeJournal, null), false);
    }

    // 11) Recalculer le solde cumulé
    calculerSoldeCumule();
...
// Lancer l’écoute Enter sur le tableau




// ──────────────────────────────────────────────────────────────────────────
// 7) Écoute la touche “Enter” dans Tabulator
//    1) Ajoute instantanément les 3 lignes (fournisseur / chargeHT / TVA)
//    2) Lance l’enregistrement en arrière-plan (pas d’await pour l’UI)
//    3) Replace le focus sur la première ligne vide (ou duplique si mode “libre”)
// ──────────────────────────────────────────────────────────────────────────
async function ecouterEntrer(table) {
  console.log("▶ ecouterEntrer() initialisé");
  table.element.addEventListener("keydown", async function (event) {
    if (event.key !== "Enter") return;
    event.preventDefault();
    console.log("⏎ Entrée pressée → flux démarré");

    // a) Récupérer la ligne active (sélectionnée)
    const selectedRows = table.getSelectedRows();
    if (!selectedRows.length) {
      console.warn("⚠️ Aucune ligne sélectionnée dans Tabulator");
      return;
    }
    const activeRowData = selectedRows[0].getData();
    console.log("▶ Ligne active sélectionnée :", activeRowData);

    // b) Ajouter instantanément les 3 lignes (fournisseur / chargeHT / TVA)
    let addedRows = await ajouterLigne(table, true, activeRowData);
    if (!Array.isArray(addedRows)) addedRows = [addedRows];
    console.log("⚡ Lignes ajoutées (instantané) :", addedRows);

    // c) Lancer l’enregistrement en fond (pas d’await pour ne pas bloquer l’UI)
    enregistrerLignesAch();

    // d) Rechercher la première ligne vide (pas de compte)
    const emptyRowComp = table.getRows().find((rowComp) => {
      const d = rowComp.getData();
      return !d.compte || d.compte.trim() === "";
    });

    // e) Si mode « libre », dupliquer la dernière ligne non vide si le solde cumulé ≠ 0
    const selectedFilter = document.querySelector('input[name="filter-achats"]:checked')?.value;
    if (selectedFilter === "libre") {
      const allData = table.getData();
      const nonEmptyRows = allData.filter((r) => r.compte && r.compte.trim() !== "");
      const lastRecorded = nonEmptyRows[nonEmptyRows.length - 1];
      if (lastRecorded) {
        const cumBalance = parseFloat(lastRecorded.value) || 0;
        if (cumBalance !== 0) {
          console.log("🔄 Mode libre & solde cumulé ≠ 0 → duplication");
          const newData = {
            id: allData.length + 1,
            ...
            piece_justificative: lastRecorded.piece_justificative,
            numero_facture: lastRecorded.numero_facture,
            libelle: lastRecorded.libelle,
            rubrique_tva: lastRecorded.rubrique_tva,
            prorat_de_deduction: lastRecorded.prorat_de_deduction,
            type_journal: lastRecorded.type_journal,
            date: lastRecorded.date,
          };
          const newRowComp = await table.addRow(newData);
          newRowComp.getElement().querySelector(".date-ligne")?.focus();
          console.log("✅ Duplication effectuée, focus sur date");
          return;
        }
      }
    }

    // f) Sinon (mode « contre-partie auto » ou aucune duplication), focus sur la ligne vide
    if (emptyRowComp) {
      emptyRowComp.getElement().querySelector(".date-ligne")?.focus();
      console.log("✅ Focus sur la première ligne vide");
    } else {
      console.warn("⚠️ Aucune ligne vide trouvée pour le focus");
    }
  });
}

// ──────────────────────────────────────────────────────────────────────────
// 8) Lancement de l’écoute sur tableAch
// ──────────────────────────────────────────────────────────────────────────
ecouterEntrer(tableAch);
...
$(document).ready(function() {
 ...
 $(document).ready(function() {
  ...
  console.log("🔍 Pièce détectée depuis URL :", piece);
  ...
  async function chargerEcritures() {
    try {
      ...
      // Injecter dans le tableau Tabulator
      tableAch.setData(lignes);

      // Remplir le <select> des codes journaux
      const journaux = [...new Set(lignes.map(l => l.type_journal))];
      const $sel = $('#journal-achats').empty();
      journaux.forEach(code => {
        $sel.append(`<option value="${code}">${code}</option>`);
      });

      // Pré-sélection du premier
      if (journaux.length) {
        $sel.val(journaux[0]);
        const lib = lignes.find(l => l.type_journal === journaux[0])?.intitule_journal || '';
  ...
  // Quand on change de journal, mettre à jour l’intitulé
  $('#journal-achats').on('change', function() {
    const code = $(this).val();
    const ligne = tableAch.getData().find(l => l.code_journal === code);
    $('#filter-intitule-achats').val(ligne?.intitule_journal || '');
    tableAch.setFilter('code_journal', '=', code);
  });
