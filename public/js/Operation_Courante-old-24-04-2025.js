...
function displayExcelData(data) {
    const fields = [
        'Date', 'numero_facture', 'Compte', 'Libellé',
        'Débit', 'Crédit', 'Contre_partie', 'compte_tva ',
        'Nature de l\'opération', 'Date lettrage', 'Contre-Partie','piece_justificatif'
    ];
...
function displayPdfData(text) {
    ...
    lines.forEach(line => {
        ...
        const row = {
            ...
            solde_cumule: columns[10] || '',
            piece_justificative: columns[11] || '',
...
var contrePartieColumn = {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupFilterEventHandlers() {
        ...
        const onglets = ['achats', 'ventes','operations-diverses','Banque','Caisse'];
    ...
    ['achats', 'ventes','operations-diverses','Banque','Caisse'].forEach(onglet => {
...

                {
                  ...
                  },
                  {
                    ...
                    formatter: function(cell, formatterParams, onRendered) {
                        ...
                        // Champ input en lecture seule (modifiable si besoin d'une autre interaction)
                        const input = `<input type='text' class='selected-file-input' placeholder='${affichage}' readonly value='${affichage}'>`;

                        return icon + input;
                    },
                    cellClick: function(e, cell) {
                        // Vérifier que l'utilisateur a bien cliqué sur l'icône
                        if (!$(e.target).hasClass('upload-icon')) {
                            return;
                        }

                        // Ouvrir directement le modal, sans attendre d'éventuelles modifications de la ligne
    ...
    cellEdited: function (cell) {
        ...
        },
        {
            ...
            editor: function(cell, onRendered, success, cancel, editorParams) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response && response.rubrique) {
                            ...
                            // Mettre à jour d'autres colonnes si nécessaire
                            cell.getRow().update({
                ...
                // Gestion des touches Entrée (pour valider sans perte de focus) et Échap (pour annuler)
                input.addEventListener("keydown", function(e){
                    if (e.keyCode === 13) { // Touche Entrée
                        success(input.value);

                        // Ne pas laisser le focus sortir de la cellule actuelle
                        // Empêcher la déselection de la cellule `rubrique_tva`
                        setTimeout(function() {
                            cell.getElement().focus();
                        }, 50); // Retarder légèrement pour éviter toute perte de focus
                    }
...
async function calculerDebit(rowData, typeLigne, credit) {
    ...
    // 🔹 **Appeler `recupererTauxTVAFromAPI()` en passant `rubrique_tva` et `compte_fournisseur`**
    const tauxTVA = await recupererTauxTVAFromAPI(rowData.rubrique_tva, rowData.compte_fournisseur);
    console.log(`📝 Calcul du débit pour ${typeLigne}: Crédit = ${credit}, Taux TVA récupéré = ${tauxTVA}%`);
...
tabulatorManager.applyToTabulator(tableVentes);
        ...
        document.getElementById("delete-row-btnVente").addEventListener("click", function () {
            let csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
