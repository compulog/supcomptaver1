...
// Fonction principale pour ajouter une ligne
function ajouterLigne(table, preRemplir = false, ligneActive = null) {
    let nouvellesLignes = [];
    ...
    if (preRemplir && ligneActive) {
        nouvellesLignes = ajouterLignePreRemplie(idCounter, ligneActive, codeJournal, moisActuel);
    } else {
        let ligneVide = ajouterLigneVide(idCounter, ligneActive, codeJournal, moisActuel);
        nouvellesLignes.push(ligneVide);
    }

    nouvellesLignes.forEach(ligne => table.addRow(ligne, false));
...
function ajouterLigneVide(idCounter, ligneActive, codeJournal, moisActuel) {
    return {
        ...
        debit: 0,
        credit: 0,
        piece: ligneActive ? ligneActive.piece : generatePiece(codeJournal, moisActuel, 1),
        type_journal: codeJournal || ''
    };
}

// Fonction pour ajouter deux lignes pré-remplies
function ajouterLignePreRemplie(idCounter, ligneActive, codeJournal, moisActuel) {
    ...
    ligne1.credit = ligneActive.credit || 0;
    ligne1.debit = 0;
    ...
    ligne2.credit = 0;
    ligne2.debit = ligneActive.credit || 0;
...
// Fonction pour calculer le débit en fonction du crédit saisi
...
// Fonction pour enregistrer une ligne
async function enregistrerLignesAch() {
    const lignes = tableAch.getData(); // Récupère toutes les lignes actuelles de la table
    const codeJournal = document.querySelector("#journal-achats").value; // Récupère le code journal sélectionné

    // Vérifier que le code journal est sélectionné
    if (!codeJournal) {
        alert("Veuillez sélectionner un code journal.");
        return;
    }

    // Filtrer les lignes valides (celles ayant une date, un numéro de facture et un compte renseignés)
    const lignesValides = lignes.filter(ligne => ligne.date && ligne.numero_facture && ligne.compte);

    // Si aucune ligne valide n'est trouvée, on ne fait rien et on arrête la fonction
    if (lignesValides.length === 0) {
        alert("Aucune ligne valide à enregistrer.");
        return;
    }

    // Ne pas afficher d'alerte ici, juste s'assurer que l'on n'envoie pas de lignes vides
    const lignesAvecDonnees = lignesValides.filter(ligne => ligne.date && ligne.numero_facture && ligne.compte && ligne.credit);

    // Si aucune ligne n'est remplie, on arrête la fonction sans rien faire (pas d'erreur)
    if (lignesAvecDonnees.length === 0) {
        return; // Rien n'est envoyé si toutes les lignes sont vides
    }

    try {
        // Calcul des débits pour toutes les lignes valides avant l'enregistrement
        for (const ligne of lignesAvecDonnees) {
            const typeLigne = ligne.contre_partie ? "ligne2" : "ligne1";
            await calculerDebit(ligne, typeLigne); // Attend que le débit soit calculé
        }

        // Préparer les données à envoyer
        const formData = lignesAvecDonnees.map(ligne => ({
            id: ligne.id || null,
            date: ligne.date,
            ...
            debit: ligne.debit,
            credit: ligne.credit,
            contre_partie: ligne.contre_partie || '',  // Si contre_partie est vide, envoie une chaîne vide
        ...
        // Récupérer le token CSRF pour sécuriser la requête
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        ...
        // Gestion des erreurs de réponse du backend
        if (!response.ok) {
            // Si le backend retourne une erreur (422), on l'ignore et ne l'affiche pas
            return; // Pas d'alerte ou d'erreur affichée si la requête échoue
        }
        ...
        alert("Une erreur est survenue lors de l'enregistrement des lignes.");
    }
}
...
// Fonction pour écouter l'événement "Enter" et ajouter une ligne
function ecouterEntrer(table) {
    table.element.addEventListener("keydown", async function (event) {
        if (event.key === "Enter") {
            ...
            const nouvellesLignes = ajouterLigne(table, true, ligneActive);

            await Promise.all(nouvellesLignes.map(async (ligne) => {
                const typeLigne = ligne.contre_partie ? "ligne2" : "ligne1";
            ...
            // Ajouter une nouvelle ligne vide à la fin de la table
