     ...
     function remplirRubriquesTva(selectId, selectedValue = null) {
        $.ajax({
            ...
            success: function(data) {
                ...
                // Ajouter l'option "Choisir une option" au début
                let defaultOption = new Option("Choisir une option", "");
                defaultOption.disabled = false; // Permet à l'utilisateur de sélectionner cette option
                ...
                // Si un selectedValue est passé, on le sélectionne, sinon l'option par défaut "Choisir une option" est affichée
                if (selectedValue) {
                    select.val(selectedValue).trigger('change');
                } else {
                    select.val('').trigger('change'); // Réinitialiser si aucun selectedValue n'est passé
                }
            },
    ...
    $(document).ready(function() {
       ...
       var table = new Tabulator("#societes-table", {
           ...
           columns: [
               ...
               {title: "Code Societe", field: "code_societe", width: 130, formatter: function(cell) {
               ...
               {title: "ICE", field: "ice", headerFilter: true, width: 130},
               {title: "RC", field: "rc", headerFilter: true, width: 70},
               {title: "Identifiant Fiscal", field: "identifiant_fiscal", headerFilter: true, width: 160},
               {
...
$(document).ready(function() {
    // Événement lors de l'ouverture du modal de modification
    $('#modifierSocieteModal').on('shown.bs.modal', function(event) {
        // Mettre le focus sur le champ "Raison sociale"
        $('#mod_raison_sociale').focus();
    });
