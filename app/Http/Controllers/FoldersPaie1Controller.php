...
class FoldersPaie1Controller extends Controller
{
    ...
    public function index($id, Request $request)
    {
        ...
        if ($societeId) {
            ...
            return view('foldersPaie1', compact('achatFiles', 'folders', 'foldersId', 'folder', 'notifications')); 
        } else {
    ...
    public function create(Request $request)
    {
        ...
        if ($request->has('folders_id') && $request->folders_id) {
            return redirect()->route('foldersPaie1', ['id' => $request->folders_id])->with('success', 'Dossier créé avec succès');
        }
