...
class CaisseController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($societeId) {
            // Filtrer les fichiers de type 'caisse' pour la société donnée
            $files = File::where('societe_id', $societeId)
                         ->where('type', 'caisse') // Filtrer par type 'caisse'
                         ->get();

            return view('caisse', compact('files')); // Passez les fichiers à la vue
        } else {
